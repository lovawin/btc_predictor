...
class PolymarketClient:
    def __init__(self, live_trading: bool, proxy_url: str = "") -> None:
        self.live_trading = live_trading
        self.proxy_url = proxy_url

    def get_token_price(
        self, market_id: str, side: str, proxy_url: str = ""
    ) -> Optional[float]:
        """Return the best ask (for YES) or best bid (for NO) token price."""
        effective_proxy = proxy_url or self.proxy_url
        try:
            from py_clob_client.client import ClobClient  # type: ignore

            with self._proxy_env(effective_proxy):
                client = ClobClient(
            ...
            if side.upper() == "YES":
                return self._best_ask_price(order_book)
            else:
                return self._best_bid_price(order_book)
        except Exception:
            return None
