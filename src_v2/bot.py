...
class TradingBot:
    ...
    async def vpnon(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if settings.vpn_wg_conf:
            ...
            if "password is required" in stderr.lower() and self._is_interface_up(wg_interface):
                await update.message.reply_text("VPN ON (WireGuard profile already active).")
                return
            if settings.vpn_mx_proxy_url:
                await update.message.reply_text(
                    f"WireGuard up failed; using MX proxy route instead. Details: {stderr or 'unknown error'}"
                )
                return
            await update.message.reply_text(
            ...
            return
        if settings.vpn_mx_proxy_url:
            await update.message.reply_text("VPN route ON (MX proxy preference).")
            return
